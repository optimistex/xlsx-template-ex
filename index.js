...
xlsx.fromFileAsync('test.xlsx').then(function (wb) {
    for (name in data) {
        if (typeof data[name] === 'string') {
            ...
            let curSheet = wb.find(`%${name}:begin%`)[0].row().sheet();

            let firstRowNum = wb.find(`%${name}:begin%`)[0].row().rowNumber() + 1;

            let lastRowNum = wb.find(`%${name}:end%`)[0].row().rowNumber();
            let repeatOffset = lastRowNum - firstRowNum;
            let totalObjCnt = data[name].length;
            let totalOffset = (totalObjCnt - 1) * repeatOffset;
            let maxColumn = curSheet.usedRange()._maxColumnNumber;
            let maxRow = curSheet.usedRange()._maxRowNumber;
            for (let r = maxRow; r > lastRowNum; r--) {
                for (let c = 1; c <= maxColumn; c++) {
                    let curVal = curSheet.row(r).cell(c).value();
                    let curStyle = curSheet.row(r).cell(c)._styleId;
            ...
            for (let i = 0; i < totalObjCnt; i++) {
                let curFirstRowNum = firstRowNum + i * repeatOffset;
                let curLastRowNum = lastRowNum + i * repeatOffset;
                let defRcnt = 0;
                for (let r = curFirstRowNum; r < curLastRowNum; r++) {
                    for (let c = 1; c <= maxColumn; c++) {
                        let tpltVal = curSheet.row(firstRowNum + defRcnt).cell(c).value();
                        let tpltStyleId = curSheet.row(firstRowNum + defRcnt).cell(c)._styleId;
